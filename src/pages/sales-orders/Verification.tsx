...
export default function Verification() {
  ...
  const columns: Column<SalesOrder>[] = [
    ...
    { key: 'received', header: 'Received At', width: '148px', sortValue: (r) => poReceivedAtOf(r) ?? '', render: (r) => { const at = poReceivedAtOf(r); return <span className="text-surface-600">{at ? formatDateTime(at) : '—'}</span>; } },
    ...
    { key: 'sla', header: 'SLA Status', width: '132px', sortValue: (r) => verificationSla(r)?.state ?? '', render: (r) => { const info = verificationSla(r); return info ? <StatusBadge tone={info.tone} label={info.label} dot /> : <span className="text-surface-400">—</span>; } },
    ...
    <>
      ...
      <div className="card">
        <div className="border-b border-surface-100 p-4">
          <FilterBar chips={chips} onClearAll={() => { setOffice(''); setStatus(''); setSla(''); setSearch(''); }}>
