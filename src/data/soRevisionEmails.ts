...
export const SO_REVISION_EMAILS: InboxEmail[] = REVISION_ORDERS.map((so, idx) => {
  ...
  return {
    ...
    extraction: [
      ...
      { key: 'linkedPoQtn', label: 'Linked PO & Quotation', value: `${so.poNumber}${so.quotationNumber ? ` / ${so.quotationNumber}` : ''}`, confidence: 'high', required: true },
      { key: 'reason', label: 'Revision Reason', value: reason, confidence: 'medium', required: true },
      { key: 'itemChanges', label: 'Requested Item Changes', value: requestedChanges.map((c) => `${c.label}: ${c.oldValue} → ${c.newValue}`).join('; ') || 'See conversation', confidence: 'medium' },
      { key: 'termChanges', label: 'Delivery / Payment / Commercial Changes', value: 'Per requested changes above', confidence: 'medium' },
    ],
