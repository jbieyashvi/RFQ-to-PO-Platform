...
export const PO_VERIFICATION_EMAILS: InboxEmail[] = SALES_ORDERS.map((so) => {
  ...
  return {
    ...
    extraction: [
      ...
      { key: 'poDate', label: 'PO Date', value: so.poDate, confidence: 'high' },
      { key: 'quotation', label: 'Linked Quotation', value: so.quotationNumber ?? '', confidence: 'high', required: true },
      { key: 'items', label: 'Items, Quantities & Rates', value: `${so.items.length} line item${so.items.length === 1 ? '' : 's'}`, confidence: 'medium' },
      { key: 'poTotal', label: 'PO Total', value: `₹${so.poValue.toLocaleString('en-IN')}`, confidence: 'high' },
      { key: 'paymentTerms', label: 'Payment Terms', value: so.paymentTerms, confidence: 'high' },
      { key: 'deliveryTerms', label: 'Delivery Terms', value: so.deliveryTerms, confidence: 'high' },
      { key: 'deliveryDate', label: 'Delivery Date', value: so.deliveryDate, confidence: 'medium' },
    ],
